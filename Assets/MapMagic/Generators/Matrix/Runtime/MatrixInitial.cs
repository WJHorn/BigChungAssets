...
namespace MapMagic.Nodes.MatrixGenerators
{
	...
	public class Import200 : Generator, IOutlet<MatrixWorld>, ISceneGizmo
	{
		...
		{
			if (stop!=null && stop.stop) return;
			if (matrixAsset == null || matrixAsset.matrix==null || !enabled) { data.RemoveProduct(this); return; }
			...
			//if active resolution matches src matrix (common case when importing map exported from this terrain)
				...
				ImportWithEnlarge(srcMatrix, dstMatrix, data.area, stop);

			else
				ImportWithDownscale(srcMatrix, dstMatrix, data.area, stop);
		...
		/// The new function, but doesn't work for some reason (no offset)
		{
			...
			int pixelMarg = Mathf.Max((int)(0.5f/ratio.x), 2); //2 initially, but increases for big scale values
