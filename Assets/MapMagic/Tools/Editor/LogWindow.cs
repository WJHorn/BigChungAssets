...
namespace Den.Tools
{
	public class LogWindow : EditorWindow
	{
		...
		public void OnGUI () 
		{
			...
			if (threadedView)
			{
				Dictionary<int,int> threadToRow = GetThreadToRows();
		...
		public Dictionary<int,int> GetThreadToRows ()
		/// Generates a lut of idName -> row number if thread view is enabled
		{
			HashSet<int> usedThreads = Log.UsedThreads();

			Dictionary<int,int> threadToRow = new Dictionary<int,int>(usedThreads.Count);
			int counter = 0;

			if (usedThreads.Contains(1))
				{ threadToRow.Add(1, 0); counter++; }

			foreach (int thread in usedThreads)
				if (!threadToRow.ContainsKey(thread))
					{ threadToRow.Add(thread, counter); counter++; }

			return threadToRow;
		}


		public void DrawHeaders (Dictionary<int,int> threadToRow)
		{
			...
			UnityEngine.GUI.BeginScrollView(
				...
				foreach (int id in threadToRow.Keys)
				{
					int rowNum = threadToRow[id];
					Rect rect = new Rect(rowNum*rowWidth,1,rowWidth,lineHeight);

					EditorGUI.LabelField(rect, "Thread "+id, style:EditorStyles.boldLabel);
				}
		...
		public void DrawThreadedValues (Dictionary<int,int> threadToRow)
		// if idToRow is null using non-threaded view (1 row)
		{
			...
			scrollPosition = UnityEngine.GUI.BeginScrollView(
				...
				foreach (Log.Entry entry in Log.AllEntries())
				{
					int rowNum = threadToRow[entry.thread];
