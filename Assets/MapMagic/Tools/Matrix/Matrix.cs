...
namespace Den.Tools.Matrices
{
	...
	public class Matrix // : Matrix2D<float>
	{
		...
		#region Per-pixel Arithmetic
			...
			//#if MM_NATIVE && (UNITY_EDITOR || !UNITY_ANDROID && !ENABLE_IL2CPP)
			//	[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CopyResized")]
			//	public static extern void CopyResized (Matrix src, CoordRect srcRect, Matrix dst, CoordRect dstRect, Vector2 sizeRatio=new Vector2());
			...
			//#else
				...
				{
					//if (sizeRatio.x < 0.00001f && sizeRatio.z < 0.00001f) 
					//	sizeRatio = new Vector2D(
					//		(float)dst.rect.size.x / (float)src.rect.size.x,
					//		(float)dst.rect.size.z / (float)src.rect.size.z );
