...
//		IEnumerable<IUnit> Layers ();
	...
	{
		bool CheckClear (TileData data);  //Called when clearing recursively from graph (no matter ready or not), returns ready state
		void ForceClear (TileData data);  //Called when this gen changed directly
	}
