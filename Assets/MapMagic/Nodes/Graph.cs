	...
	public class Graph : ScriptableObject , ISerializationCallbackReceiver
	{
		...
		#region Generate
			...
			/// Sets gen to non-ready (in any sub-data) and then removes ready state for all dependent from it
			{
				...
				foreach ((Graph subGraph, TileData subData) in SubGraphsDatas(this, data))
					if (subGraph.ContainsGenerator(gen))
					{
						if (gen is ICustomClear cgen)
							cgen.ForceClear(subData);
				...
				ClearChanged(data); //only top-level data - will process inner ones with fn ClearChanged
			}
			...
			private bool ClearChangedRecursive (Generator gen, TileData data, Dictionary<Generator,bool> processed)
			{
				...
				if (gen is ICustomClear cgen)
				{
					bool customReady = cgen.CheckClear(data); //should run anyways no matter of preceding changes
			...
			{
				#if MM_DEBUG
				Log.Add("Generate (draft:" + data.isDraft + ")");
				#endif
			...
			/// For each inlet in graph writes the id of linked outlet
			{
				...
				foreach (IUnit unit in AllUnits())
					if (unit is IInlet<object> inlet)
